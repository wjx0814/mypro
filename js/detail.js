...
function ajaxData(term,smalls,loucdata,lousubdata){
	...
	// 购物操作
	// 点击添加商品数量
	var spnum= document.querySelectorAll(".num")[0];
	var spnumdata=parseInt(spnum.innerHTML);
	// 添加
	var plus=document.querySelectorAll(".plus")[0];
	plus.onclick=function(){
		spnumdata++;
		spnum.innerHTML=spnumdata;
	}
	...
	//取出所有按钮
	var btns=document.querySelectorAll(".buyNub-buy");
	//点击添加
	for(var i=0;i<btns.length;i++){
		btns[i].addEventListener("click",function(){
			...
			obj.imgsrc=imgidname.src;
			// 名称
			var lblName=document.getElementById("lblName");
			obj.name=lblName.innerHTML;
			// 积分
			var integral=document.getElementById("integral");
			obj.des=integral.innerHTML;
			// 价格
			var idmon=document.getElementById("idmon");
			obj.price=idmon.innerHTML;
			// 数量
			obj.pcount=spnumdata;//商品数量
			// 获取id
			obj.pid=imgidname.getAttribute("pid");
